...
1. 引用转换器脚本文件
2. 调用 `drewmarkConverter({params})` 函数
3. 获得转换结果字符串

```html
<script src="js/drewmark-converter.min.js"></script> <!-- 引用朱码 JS 转换器 -->
<script>
...
1. 在 `<head>` 中引用转换器的样式表文件
2. 引用转换器脚本文件
3. 确定要加载转换器的容器元素的 `id` 属性值
4. 使用 `drewmarkConverter({converter_id: 'target-id`})` 函数加载转换器的 UI 界面
...
<head>
  <meta charset="UTF-8">
  <link rel="stylesheet" href="css/drewmark-converter.min.css"> <!-- 引用转换器器的样式文件 -->
</head>
<body>
  <div id="my-converter"></div>  <!-- 用于加载转换器的容器元素 -->
  <script src="js/drewmark-converter.min.js"></script> <!-- 引用朱码 JS 转换器 -->
  <script>
    drewmarkConverter({ converter_id: 'my-converter' }); // 将转换器加载到 id="my-converter" 的容器中
  </script>
</body>
</html>
```
...
+ *UI 模式*同样支持*无 UI 模式*的三个参数，并作为初始值加载 UI 界面。
